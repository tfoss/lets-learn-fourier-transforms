...
- If `bd` is not installed (common in Docker), install it: `curl -fsSL https://raw.githubusercontent.com/steveyegge/beads/main/scripts/install.sh | bash`
...
- **Docker**: The `bd` wrapper in Docker connects to the host's dolt server via `host.docker.internal`. If `bd` fails, the host's dolt server may not be running — run `bd list` on the host first to start it.
